...
//printf("%ld %ld\n",(long int)tp1.tv_sec,tp1.tv_nsec);
  ...
  // Performing the convolution only on the columns
for (r=0; r<ROWS; r++)
  for (c=0; c<COLS; c++)
  {
    sum1 = 0.0;
    if ( c<3 || c>=(COLS-3) ){ // If the kernel goes outside the image, then make the pixel value zero.
        sum1=0.0;
...
//Performing the convolution on the rows
for (r=0; r<ROWS; r++)
  for (c=0; c<COLS; c++)
  {
    sum = 0.0;
    if ( r<3 || r>=(ROWS-3)){ // If the kernel goes outside the image, then make the pixel value zero.
        sum=0.0;
    } else { // Otherwise perform the convolution operation
      for(r2=-3; r2<=3; r2++){
